...
sealed class Hole private constructor(
    ...
    open val radius: Double? = null,
    open val points: List<LatLng>? = null,
) {
    abstract fun mapToHoleOptions(): BaseHoleOptions
     ...
     */
    data class CircleHole(override val center: LatLng, override val radius: Double) :
        Hole(center, radius) {
    ...
    data class PolygonHole(override val points: List<LatLng>) : Hole(points = points) {
        override fun mapToHoleOptions(): PolygonHoleOptions = PolygonHoleOptions().addAll(points)
    }
}


object HoleCreator {
    fun create(center: LatLng, radius: Double): Hole.CircleHole {
